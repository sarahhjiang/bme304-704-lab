# Lab 01–02 Recap — Lectures 1–6 (Lines & Curves) -- Filled in
...
## 2. OLS: from calculus to matrices (~3 min)

The whole derivation is: **assume a model, write the error, minimize it.**
...
$$= \sum_{i=1}^{n} x_i^2 - n\bar x^2 \quad \checkmark$$
 
$S_{xy}$
 
$$S_{xy} = \sum_{i=1}^{n}\left(x_i - \bar x\right)\left(y_i - \bar y\right)$$
 
$$= \sum_{i=1}^{n}\left(x_i y_i + \bar x \bar y - \bar x y_i - x_i \bar y\right)$$
 
$$= \sum_{i=1}^{n} x_i y_i + \sum_{i=1}^{n}\bar x \bar y - \bar x \sum_{i=1}^{n} y_i - \bar y \sum_{i=1}^{n} x_i$$
 
$$= \sum_{i=1}^{n} x_i y_i + n\bar x \bar y - \bar x n \bar y - \bar y n \bar x$$
